...
const initialCategories = [
  { id: 1, name: 'Herbs',role:'homemaker',  content: 'Pure ayurvedic herbs and plant extracts' },
  { id: 2, name: 'Supplements',role:'homemaker',  content: 'Dietary supplements and vitamins' },
  { id: 3, name: 'Oils',role:'homemaker',  content: 'Therapeutic and massage oils' },
  { id: 4, name: 'Teas',role:'homemaker', content: 'Herbal tea blends and infusions' },
];
            ...
            <form method='post' onSubmit={handleFormSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input 
                    type="file" 
                    className="w-full file:bg-[#0E2A10] file:mr-4 file:text-amber-50 file:px-4 file:py-2 border border-[#87a186]/30 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0E2A10]/30 bg-white"
                    value={formData.name}
                    onChange={(e) => setFormData({...formData, name: e.target.value})}
                    required
                  />
          ...
          <table className="w-full min-w-full text-left">
            <thead>
              <tr>
                ...
                <th className='bg-[#0E2A10]/80 text-white font-medium px-6 py-3'>Role</th>
                <th className='bg-[#0E2A10]/80 text-white font-medium px-6 py-3'>Content</th>
              ...
              {filteredCategories.map((category) => (
                <tr key={category.id}>
                  ...
                  <td className='border-t border-[#87a186]/10 px-6 py-4'>
                    <img  src={category.image} alt={category.name} className="w-12 h-12 rounded-full object-cover border" />
                  </td>
                  <td className="font-medium text-[#0E2A10] border-t border-[#87a186]/10 px-6 py-4 ">{category.name}</td>
                  <td className='border-t border-[#87a186]/10 px-6 py-4'>{category.role}</td>
                  <td className='border-t border-[#87a186]/10 px-6 py-4'>{category.content}</td>
                  <td className='border-t border-[#87a186]/10 px-6 py-4'>
